...
namespace pr {
    ...
    class PlateInfo {
        public:
            ...
            float confidence = 0;
            PlateInfo(const cv::Mat &plateData, std::string plateName, cv::Rect plateRect, int plateType) {
                    licensePlate = plateData;
                    name = plateName;
                    ROI = plateRect;
                    Type = plateType;
                }
            PlateInfo(const cv::Mat &plateData, cv::Rect plateRect, int plateType) {
                licensePlate = plateData;
                ROI = plateRect;
                Type = plateType;
            }
